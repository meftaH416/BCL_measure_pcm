...
class IDF_EDITING < OpenStudio::Measure::EnergyPlusMeasure
  ...
  def run(workspace, runner, user_arguments)
    ...
    # Iterate over the constructions to find the target envelope
    constructions.each do |construction|
      if construction.getString(0).to_s == envelope_name # (0) is the name field
        layers = construction.getString(1).to_s.split(",")
        layers.insert(pcm_index + 1, pcm_name)    # Insert PCM material at the correct index
        construction.setString(1, layers.join(","))  # Update the layers field
        break
      end
    end
