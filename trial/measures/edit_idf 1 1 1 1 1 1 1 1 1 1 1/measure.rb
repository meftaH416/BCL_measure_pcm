...
class EditIdf < OpenStudio::Measure::EnergyPlusMeasure
  ...
  def arguments(workspace)
    ...
    pcm_index = OpenStudio::Measure::OSArgument.makeStringArgument('pcm_index', true)
  ...
  def run(workspace, runner, user_arguments)
    ...
    pcm_index = runner.getStringArgument('pcm_index', user_arguments)
    ...
    # add PCM to layer of the envelope
    constructions = workspace.getObjectsByType("Construction".to_IddObjectType)

   # get names of constructions used in the model
    constructions.each do |construction|
      if construction.getString(0).to_s == envelope_name #(0) is name
         construction.insert(pcm_index+1, pcm_name)
         break
      end
    end
